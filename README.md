...
Smile Storm 是一个独立的浏览器端直播打赏 AR 原型：微笑时持续下雨，大笑时停止下雨并发射烟花；烟花粒子会与实时头部轮廓碰撞，用户可以摆头把粒子撞开。完整摇头会在画面右侧同一位置切换展示墨镜与帽子，每次仅显示 2 秒；天气特效期间还会低概率掉落有真实厚度与头部前后遮挡关系的 Three.js 行星环，每次只展示 1 秒。
...
4. 完成一次左右摇头后在画面右侧显示浅黑墨镜，后续每次完整摇头在同一位置切换墨镜和帽子；每件显示 2 秒后自动卸载。星轨无需点选：下雨时按 6%/秒、烟花按 18%/次抽取；命中后显示 1 秒，并有 3 秒冷却。
...
- AR 配饰：完整左右摇头通过 MediaPipe 变换矩阵的 yaw 识别，避免把人脸横向移动误判为摇头；首次显示墨镜，之后在墨镜/帽子间切换。两件 GLB 约 10k/12k 三角面、合计约 297KB，按需加载到固定侧边层，2 秒到期后从场景移除并释放几何、材质与纹理。

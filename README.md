...
Smile Storm 是一个独立的浏览器端直播打赏 AR 原型：微笑时持续下雨，大笑时停止下雨并发射烟花；烟花粒子会与实时头部轮廓碰撞，用户可以摆头把粒子撞开。轻幅左右摇头会在画面右侧同一位置切换展示墨镜与帽子；模型缓慢自转并在 2 秒尾段平滑淡出，随后立即释放。天气特效期间还会低概率掉落有真实厚度与头部前后遮挡关系的 Three.js 行星环，每次只展示 1 秒。
...
4. 完成一次轻幅左右摇头后在画面右侧显示浅黑墨镜，后续每次摇头在同一位置切换墨镜和帽子；每件缓慢自转，显示 2 秒并在尾段淡出后自动卸载。星轨无需点选：下雨时按 6%/秒、烟花按 18%/次抽取；命中后显示 1 秒，并有 3 秒冷却。
...
- AR 配饰：轻幅左右摇头通过 MediaPipe 变换矩阵的 yaw 识别，默认阈值约 6.9°，避免把人脸横向移动误判为摇头；首次显示墨镜，之后在墨镜/帽子间切换。两件 GLB 约 10k/12k 三角面、合计约 297KB，按需加载到固定侧边层，复用主渲染循环缓慢自转，最后 0.6 秒平滑淡出，2 秒到期后从场景移除并释放几何、材质与纹理。
- AR 礼物：保留由倾斜 Torus 与低模行星构成的星轨光环。一个不可见、只写深度的低模头部椭球让配饰后部被真人头部自然遮挡，前部仍可见。
- 掉落规则：正式页面没有礼物佩戴或切换按钮；只有下雨（6%/秒）或烟花（18%/次）命中时激活星轨光环，严格持续 1000ms，随后进入 3 秒冷却。`?debug=1` 中保留强制预览按钮，仅用于本地验收。
- 性能预算：星轨、墨镜、帽子与粒子共用同一个 WebGLRenderer 和同一深度缓冲，不创建第二个 WebGL 上下文；同一时间只显示一件穿戴配饰，LOW 档减少星轨行星数量。
...
- `app/lib/head-accessories.ts`：GLB 单次加载、头部深度遮挡、锚定、质量降级和资源释放
- `app/lib/head-shake-controller.ts`：完整摇头识别、配饰切换与 2 秒生命周期
- `app/lib/accessory-drop-controller.ts`：天气事件概率、1000ms 生命周期和 3 秒冷却
- `app/lib/effect-audio.ts`：一次性音效、循环声、无漏音预热与无 React 重渲染淡出

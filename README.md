...
- AR 礼物：仅保留由倾斜 Torus 与低模行星构成的星轨光环。一个不可见、只写深度的低模头部椭球让星轨后半部被真人头部自然遮挡，前半部仍可见。
- 掉落规则：正式页面没有礼物佩戴或切换按钮；只有下雨（6%/秒）或烟花（18%/次）命中时激活星轨光环，严格持续 1000ms，随后进入 3 秒冷却。`?debug=1` 中保留强制预览按钮，仅用于本地验收。
- 性能预算：星轨光环与粒子共用同一个 WebGLRenderer 和同一深度缓冲，不创建第二个 WebGL 上下文；资产三角面低于 1800，LOW 档减少行星数量。
